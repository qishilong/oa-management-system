...
2. **定义饼状图组建，进行界面饼状图渲染**
    ...
    ```js
    const Pie = ({ title, renderList, styleData, showSideBar = false, isEmpty = false, isArea=false }) => {
...
3. **创建柱状图组件**
    ...
    ```js
    const Columnar = ({ title, renderData, styleData }) => {
      const option = {
        title: { text: title, },
    ...
    <div className="staff-amount-container" style={{ ...styleData }}>
          ...
          <div className="staff-amount_main" >
            <div className="staff-header">
              <span className="staff-list-title">姓名</span>
              <span className="staff-list-title">公司</span>
            </div>
              ...
              {renderList?.map((item, index) => {
                return (
                  <div className="staff-list-item" key={index}>
                    ...
                    <span className="item">{item.department ? item.department : <span className="danger"> 未绑定部门</span>}</span>
                  </div>
                )
              })}
            </div>
